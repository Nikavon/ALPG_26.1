...
livros_restritos = ["1984", "duna", "fundacao"]

livro_solicitado = input("Qual livro você deseja pegar emprestado? ").lower()
tempo_emprestimo = int(input("Quantos dias você pretende ficar com o livro? "))

if livro_solicitado in livros_restritos or tempo_emprestimo > 14:
    print("Empréstimo negado: Livro restrito ou prazo excede o limite permitido")
else:
    multas_pendentes = input("Você possui multas pendentes na biblioteca? (sim/nao) ").lower()
    if multas_pendentes == "sim":
        print("Empréstimo bloqueado devido a pendências financeiras.")
    else:
        print(f"Empréstimo aprovado! Devolva em {tempo_emprestimo} dias.")    